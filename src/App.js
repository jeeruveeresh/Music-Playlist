...
class App extends Component {
  state = {
    updatedData: initialTracksList,
    searchInput: '',
  }

  onClickDeleteUser = id => {
    const {updatedData} = this.state
    const filterData = updatedData.filter(eachItem => eachItem.id !== id)
    this.setState({updatedData: filterData})
  }
  ...
  render() {
    const {updatedData, searchInput} = this.state
    const searchData = updatedData.filter(search =>
        ...
        <div className="app-container">
          ...
          <div className="details-container">
            ...
            <ul className="songs-list">
              {searchData.length === 0 ? (
                <p className="description">No Songs Found</p>
              ) : (
